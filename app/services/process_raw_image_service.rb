class ProcessRawImageService
  def initialize(vision_data, upload)
    @doc_text_bound = vision_data.document.bounds
    @word = vision_data.document.words
    @upload = upload
  end
  ...
  def call
    ...
    processed_img = image.write "app/assets/images/#{new_image.processed_photo}"

    p_image = vision.image("app/assets/images/#{new_image.processed_photo}")
    p_image
  end
end